...
> 同时支持 MoviePilot **V2（v2.15.0+）** 与 **V3（v3.0.0+）**。
> - V2 版本：`v1.0.6`（`plugins.v2/mediamissingsubscribe_me/`）
> - V3 版本：`v2.0.6`（`plugins.v3/mediamissingsubscribe_me/`，按官方迁移规范完成 V3 独立适配）
>
...
- 单条操作：订阅 / 忽略 / 恢复 / 删除；发现新的合集缺失电影时支持系统通知（文案按合集聚合、最多三行，不会被片名刷屏）
